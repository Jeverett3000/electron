...
class Platform:
  ...
  def get_current():
    ...
    raise AssertionError(f"unexpected current platform '{platform}'")
...
class TestsList():
  ...
  def __merge_nested_lists(value):
    ...
    raise AssertionError(f"unexpected type for list merging: {type(value)}")
...
class TestBinary():
  ...
  def run(self, included_tests=None, excluded_tests=None,
    ...
    try:
      ...
      if Verbosity.ge(verbosity, Verbosity.ERRORS):
        print(
            f"An error occurred while running '{self.binary_path}':",
            '\n',
            exception,
            file=sys.stderr,
        )
  ...
  def __get_gtest_filter(included_tests, excluded_tests):
    ...
    return f"--gtest_filter={included_tests_string}-{excluded_tests_string}"
